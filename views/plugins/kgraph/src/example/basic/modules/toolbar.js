...
const Toolbar = function (graph, refs = {}) {
  ...
  const updateHistoryTools = function () {
    let stateId = $state.getStateId(),
      stateCount = $state.getLength();
  ...
  const update = function () {
    tools.list.forEach((t) =>
      t.dom[t.config.disabled ? "addClass" : "removeClass"]("disabled")
    );
  };
  ...
  const createTool = function (t) {
    let tool = newElement({
      ...
      props: {
        title: t.title,
        className: "iconfont icon-" + t.key + " disabled",
      },
      evts: {
        click: function () {
  ...
  const createTools = function () {
    newElement(
      { tag: "div", ref: "tools", props: { className: "kgraph-toolbar" } },
      refs
    );

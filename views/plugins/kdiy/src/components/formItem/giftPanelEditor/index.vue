/**
 * @author liFeng
 * @email feng.li@arvato.com
 */
<template>
  <el-row>
    <el-col>
      <el-row
        type="flex"
        align="middle"
        :gutter="20"
        v-for="item,index in data"
        :key="item[ prop.keyword1 ].keys"
      >
        <el-col style="width:770px">
          <div class="orderly-list-box">
            <quickForm :model="item" ref="form"></quickForm>
            <i
              class="el-icon-circle-close close"
              v-if="data.length != 1"
              @click="onDeleteOrderlyList(index)"
            ></i>
          </div>
        </el-col>
      </el-row>
    </el-col>
    <el-col>
      <el-button @click="onInsertOrderlyList">添加优惠</el-button>
    </el-col>
  </el-row>
</template>

<script>
import quickForm from "@/components/quickForm";
import quickFormItem from "@/components/quickFormItem";

const relationMap = {
  or: "or",
  and: "and"
};

export default {
  name: "giftPanelEditor",
  props: ["value", "prop", "config", "options"],
  data() {
    const vm = this,
      r = this.$rules;

    return {
      index: 0,
      isAdd: false,
      model: []
    };
  },
  computed: {
    data() {
      const fresh = this.toDataRegroup(this.model);

      this.$emit("input", fresh);
      return this.model;
    }
  },
  created() {
    const { keyword1, keyword2, keyword3, keyword4 } = this.prop;

    this.model = this.value.map(item =>
      this.getOrderlyList([
        item[keyword1],
        item[keyword2],
        item[keyword3],
        item[keyword4]
      ])
    );
  },
  methods: {
    onDeleteOrderlyList(index) {
      this.model.splice(index, 1);
    },
    onInsertOrderlyList() {
      this.model.push(this.getOrderlyList());
    },
    getOrderlyList(values = []) {
      const vm = this;

      const { keyword1, keyword2, keyword3, keyword4 } = this.prop;

      const {
        keyword1: keyword1_config = {},
        keyword2: keyword2_config = {},
        keyword3: keyword3_config = {},
        keyword4: keyword4_config = {}
      } = this.config || {};

      const [v1 = "", v2 = [], v3 = {}, v4 = ""] = values;

      const r = this.$rules;

      const TODOLIST_TEMPLATE = {
        [keyword3]: {
          label: "",
          labelWidth: "0px",
          custom: true,
          type: "giftConditionPanel",
          value: v3,
          props: {
            prop: {
              keyword1: "andOr",
              keyword2: "orderAmount",
              keyword3: "orderNum"
            },
            options: this.options
          },
          ...keyword3_config
        },
        [keyword4]: {
          type: "input",
          label: "选赠",
          value: v4,
          rules: [r.required(), r.nInt()],
          props: {
            style: "width:140px !important",
            placeholder: "请输入选赠数量"
          },
          ...keyword4_config
        },
        [keyword1]: {
          keys: this.index++,
          type: "input",
          label: "排序",
          value: v1,
          props: {
            style: "width:140px !important",
          },
          rules: [r.required(), r.nInt()],
          ...keyword1_config
        },
        [keyword2]: {
          custom: true,
          type: "selector",
          label: "选择赠品",
          value: v2,
          props: {
            label: "添加赠品",
            type: "gift",
            filter: {
              status: 1
            }
          },
          ...keyword2_config
        }
      };

      return TODOLIST_TEMPLATE;
    },
    toDataRegroup(data = []) {
      const { keyword1, keyword2, keyword3, keyword4 } = this.prop;

      const fresh = data.map(item => {
        return {
          [keyword1]: item[keyword1].value,
          [keyword2]: item[keyword2].value,
          [keyword3]: item[keyword3].value,
          [keyword4]: item[keyword4].value
        };
      });
      console.log(123123123123123123123123123);
      return fresh;
    },
    validate(callback) {
      const { prop, model } = this;
      const { form } = this.$refs;

      const promise = new Promise(function(resolve, reject) {
        callback = function(valid) {
          valid === true ? resolve(valid) : reject(valid);
        };
      });

      let arr = [];

      form.forEach(item => {
        const formOne = item.$refs[prop.keyword3][0].getInput(prop.keyword3)
          .$children[0];

        formOne.validate(valid => {
          arr.push(valid);
        });

        item.validate(valid => {
          arr.push(valid);
        });
      });

      if (arr.includes(false)) {
        callback(false);
      } else {
        for (let i = 0; i < model.length; i++) {
          const val = model[i][prop.keyword2].value;
          if (val.length == 0) {
            callback({ msg: "赠品不能为空" });
            return promise;
          }
        }
        callback(true);
      }

      return promise;
    }
  },
  components: {
    quickForm,
    quickFormItem
  }
};
</script>

<style lang="scss" scoped>
.orderly-list-box {
  padding: 20px 30px 0;
  border: 1px dashed #ccc;
  margin: 10px 0;
  position: relative;

  .close {
    position: absolute;
    right: -12px;
    top: -12px;
    font-size: 24px;
    color: #999;
  }
}
</style>
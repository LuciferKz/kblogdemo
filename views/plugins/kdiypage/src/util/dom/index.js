...
const addWheelEvent = function (el, cb, config) {
  var evt =
    "wheel" in document.createElement("div")
      ? "wheel"
...
const multiElSetter = {
  ...
  find(el, selector) {
    return new KElement(el.querySelectorAll(selector));
  },
  append(el, k) {
  ...
  insertBefore(el, k) {
    for (let i = 0, len = k.length(); i < len; i++) {
      k.dom[i].parentNode.insertBefore(el, k.dom[i]);
    }
    return this;
  },
  remove(el) {
...
export const newElement = function (elm, refs, cb) {
  ...
  if (elm.children) {
    if (elm.children instanceof Array) {
      elm.children.forEach((child) => {
        const newEl = newElement(child, refs, cb);
        k.append(newEl);
      });
    } else if (typeof elm.children === "string") {
      k.html(elm.children);
    }
  }

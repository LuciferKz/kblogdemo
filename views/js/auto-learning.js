...
!(function autoLearning () {
  ...
  const goNextStep = function () {
    ...
    const activeLink = document.querySelector('.posCatalog_active .posCatalog_name')
      ...
      iframes.forEach((iframe) => {
        ...
        if (!videoIframes || videoIframes.length === 0) {
          ...
          videoIframes.forEach(vIframe => {
            const videos = vIframe.contentWindow.document.querySelectorAll('video')
    ...
    const _perm = function (source, result) {
      if (source.length == 0) {
        ...
        for (let i = 0; i < source.length; i++) {
          _perm(source.slice(0, i).concat(source.slice(i + 1)), result.concat(source[i]));
        }
      }
    }
    _perm(arr, [])

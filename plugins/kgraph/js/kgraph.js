const KGraph = function (container) {
  ...
  Diagram.prototype = {
    ...
    initCanvas: function () {
      ...
      g.canvas.height = clientRect.height - 50;
  ...
  DNode.prototype = {
    ...
    draw: function (connecting) {
      ...
      dn.entering && dn.drawConnectsBtn();
    ...
    drawConnectsBtn: function () {
      let dn = this, ctx = dn.ctx,
      
      centreX = dn.x + 120,
      centreY = dn.y + dn.height / 2,
      r = 6;
      ...
      ctx.beginPath();
      ctx.moveTo(centreX - (r - 2), centreY);
      ctx.lineTo(centreX + (r - 2), centreY);
      ctx.stroke();
  
      ctx.beginPath();
      ctx.moveTo(centreX, centreY - (r - 2));
      ctx.lineTo(centreX, centreY + (r - 2));
      ctx.stroke();
